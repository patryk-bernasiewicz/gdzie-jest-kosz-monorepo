...
describe('ClerkAuthGuard', () => {
  ...
  beforeEach(() => {
    ...
    guard = new ClerkAuthGuard(userService as any);
  ...
  it('should return false if Clerk session retrieval fails', async () => {
    request.headers = { authorization: 'Bearer token123' };
    (clerkClient.verifyToken as jest.Mock).mockResolvedValue({ sid: 'sid123' });
    (clerkClient.sessions.getSession as jest.Mock).mockRejectedValue(
