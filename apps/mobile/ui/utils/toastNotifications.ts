...
export function handleApiError(
  ...
  if (isClerkAPIResponseError(error)) {
    ...
    if (firstError) {
      ...
      if (
        ...
        firstError.meta?.paramName ===
          '__clerk_form_error_code_network_error' ||
        error.message?.toLowerCase().includes('network')
      ) {
        title = 'Błąd sieci Clerk';
        message =
          'Nie udało się połączyć z serwerem Clerk. Sprawdź połączenie internetowe.';
      }
    ...
    if (error.message?.toLowerCase().includes('network')) {
      title = 'Błąd sieci Clerk';
      message =
        'Problem z połączeniem podczas operacji Clerk. Sprawdź połączenie internetowe.';
    }
  } else if (
    typeof error === 'object' &&
    error !== null &&
    ...
    if (
      ...
      title = 'Błąd sieci';
      message =
        'Przekroczono czas oczekiwania na odpowiedź lub problem z siecią.';
    } else {
      message = err.message;
    }
  }
